...
def test_bot_note_clears_merge_error(project: Project) -> None:
    """Bot-authored notes signal new activity and trigger merge-error
    removal so the integration retries."""
    mr = _make_healthcheck_mr(labels=["lgtm", "merge-error"])
    label_event = create_autospec(ProjectMergeRequestResourceLabelEvent)
    label_event.action = "add"
    label_event.label = {"name": "merge-error"}
...
def test_merge_error_applied_when_cannot_be_merged(project: Project) -> None:
    """merge-error is applied to queue-eligible MRs stuck in
    cannot-be-merged state (e.g. after a failed async rebase)."""
    mr = _make_healthcheck_mr(labels=["lgtm"], merge_status="cannot_be_merged")
    mocked_gl = create_autospec(GitLabApi)
    mocked_gl.project = project
...
def test_merge_error_not_duplicated_when_cannot_be_merged(project: Project) -> None:
    """merge-error is not re-applied if the MR already has it."""
    mr = _make_healthcheck_mr(
        labels=["lgtm", "merge-error"], merge_status="cannot_be_merged"
    )
    mocked_gl = create_autospec(GitLabApi)
    mocked_gl.project = project
...
def test_cannot_be_merged_skipped_when_not_queue_eligible(project: Project) -> None:
    """MRs in cannot-be-merged state without approval labels are
    still skipped -- merge-error is only for queue-eligible MRs."""
    mr = _make_healthcheck_mr(labels=["some-label"], merge_status="cannot_be_merged")
    mocked_gl = create_autospec(GitLabApi)
    mocked_gl.project = project
